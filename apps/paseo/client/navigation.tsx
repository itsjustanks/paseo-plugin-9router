...
export function Overview({ theme, compact, data, onSelect, refresh, refreshing, openDashboard, dashboardBusy }: { theme: Theme; compact: boolean; data?: RouterStatus; onSelect: (tab: TabId) => void; refresh: () => void; refreshing: boolean; openDashboard: () => void; dashboardBusy: boolean }) {
  ...
  return <>
    ...
    <Card theme={theme}><Chip theme={theme} label="Your next step" tone="success" /><Step theme={theme} index={0} title={next.title} /><Note theme={theme}>{next.detail}</Note><View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}><Button theme={theme} label={next.label} tone="primary" onPress={() => onSelect(next.tab)} /><Button theme={theme} label="Open dashboard" busy={dashboardBusy} onPress={openDashboard} /><Button theme={theme} label="Read the walkthrough" onPress={() => onSelect("guide")} /><Button theme={theme} label="Refresh status" busy={refreshing} onPress={refresh} /></View></Card>
